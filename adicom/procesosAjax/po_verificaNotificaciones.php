...
$TXTSQL = "select * from oficios inner join archivos on folio=oficio where abogado_solicitante = '".$usuario."' and tipo='opi_legal' and atendido='1' and visto = '0'";								 ;
...
{
	$tipo = $r['tipo'];
	if($tipo == "sa") { $imagen = "FAQ.png"; $estilo = "olLink";}
...
if($direccion == 'DG')
	$sql = $conexion->select("SELECT num_accion, fecha_analisis_documentacion, limite_cierre_instruccion, (DATEDIFF( CURDATE( ) , limite_cierre_instruccion) * -1) AS DifDias
								...
								(detalle_edo_tramite <> 29 AND detalle_edo_tramite <> 23 AND detalle_edo_tramite <> 24 AND detalle_edo_tramite <> 25 AND detalle_edo_tramite <> 26 AND detalle_edo_tramite <> 45) 
							  ORDER BY DifDias", false);
else
	$sql = $conexion->select("SELECT num_accion, fecha_analisis_documentacion, limite_cierre_instruccion, (DATEDIFF( CURDATE( ) , limite_cierre_instruccion) * -1) as DifDias  
								FROM pfrr 
								WHERE  (DATEDIFF( CURDATE( ) , limite_cierre_instruccion) <=30 || DATEDIFF( CURDATE( ) , limite_cierre_instruccion) <=60 || DATEDIFF( CURDATE( ) , limite_cierre_instruccion) <=90) AND 
								(detalle_edo_tramite <> 29 AND detalle_edo_tramite <> 23 AND detalle_edo_tramite <> 24 AND detalle_edo_tramite <> 25 AND detalle_edo_tramite <> 26 AND detalle_edo_tramite <> 45)  AND 
								subnivel IN (".$niveles.") 
							  ORDER BY DifDias", false);
/*
...
if($direccion == 'DG')
	$sqlsumpo = $conexion->select("SELECT `num_accion`, `detalle_de_estado_de_tramite`, `fecha_estado_tramite`, 
									(DATEDIFF( CURDATE( ) , `fecha_estado_tramite`)) AS sumDias 
								   from po 
								   WHERE `detalle_de_estado_de_tramite` = 5 
							  ORDER BY sumDias", false);
else
	$sqlsumpo = $conexion->select("SELECT `num_accion`, `detalle_de_estado_de_tramite`, `fecha_estado_tramite`, 
									(DATEDIFF( CURDATE( ) , `fecha_estado_tramite`)) AS sumDias 
								   from po 
								   WHERE `detalle_de_estado_de_tramite` = 5
								   AND subnivel IN (".$niveles.") 
							  ORDER BY sumDias", false);


while($sumpo = mysqli_fetch_array($sqlsumpo))
{
	if($sumpo['sumDias'] == 1 ){ $ndia='día';}else{ $ndia='días'; }

		$linksPO[] = array('fecha' => $sumpo['fecha_estado_tramite'],'titulo' => "Proceso de Notificación ",'link' => "
		<li>
			<a class='linkNoti poLink' href=\"#\" onclick=\"var cuadro3 = new mostrarCuadro(250,700,'Proceso de Notificación " .$sumpo['num_accion']."',100,'cont/po_enpro.php','numAccion=".$sumpo['num_accion']."&fecha=".$sumpo['fecha_estado_tramite']."&sumad=".$sumpo['sumDias']."')\"> 
				<div style='float:left'> <img src='images/Apps-preferences-system-time-icon.png' /> </div> 
				<div style='float:left'> Proceso de Notificación <br>".$sumpo['num_accion']." </div> 
				<div style='float:right; padding:0px 5px 0 0;'>Total <br>".$sumpo['sumDias']." ".$ndia." </div> 
			</a> 
			</div> 
			<div class='clear'>
...
///----------------------NOTIFICACIÓN DE MONTO DE INICIO---------------/

if($direccion == 'DG')
	$sqlinipfrr = $conexion->select("SELECT `num_accion`, `detalle_edo_tramite`, `inicio_frr` 
								   from pfrr
								   WHERE `detalle_edo_tramite` = 30
								   AND `inicio_frr` = ''
							  ORDER BY num_accion", false);
else
	$sqlinipfrr = $conexion->select("SELECT `num_accion`, `detalle_edo_tramite`, `inicio_frr` 
								   from pfrr 
								   WHERE `detalle_edo_tramite` = 30
...
while($inipfrr = mysqli_fetch_array($sqlinipfrr))
{

		$linksPFRRINI[] = array('fecha' => $inipfrr['fecha_edo_tramite'],'titulo' => "Monto de Inicio ",'link' => "
		<li>
			<a class='linkNoti pfrrLink' href=\"#\" onclick=\"var cuadro3 = new mostrarCuadro(250,700,'Monto de Inicio " .$inipfrr['num_accion']."',100,'cont/pfrr_montoini.php','numAccion=".$inipfrr['num_accion']."&fecha=".$inipfrr['fecha_edo_tramite']."')\"> 
				<div style='float:left'> <img src='images/Apps-preferences-system-time-icon.png' /> </div> 
				<div style='float:left'> Monto de Inicio <br>".$inipfrr['num_accion']." </div> 
				<div style='float:right; padding:0px 5px 0 0;'>Pendiente. <br> </div> 
			</a> 
			</div> 
			<div class='clear'>
...
$numsLinks6 = count($linksPO);
$numsLinks7 = count($linksPFRRINI);


if($linksAud > 0 || $linksAyuda > 0 || $linksCrals > 0 || $linksLimites > 0 || $linksUAA > 0 || $linksPO > 0 || $linksPFRRINI > 0)
{
	...
	@usort($linksPO, 'ordenar');
	@usort($linksPFRRINI, 'ordenar');
	...
	$pen = $numsLinks1+$numsLinks2+$numsLinks3+$numsLinks4+$numsLinks5+$numsLinks6+$numsLinks7;
	
	if($pen == '' || $pen == 0 || $pen == -1) $pen = 0;
	
	echo $resTxt = @mostrar_array($linksAyuda).@mostrar_array($linksPFRRINI).@mostrar_array($linksCrals).@mostrar_array($linksPO).@mostrar_array($linksAud).@mostrar_array($linksLimites).@mostrar_array($linksUAA)."|".$pen;
