...
$txtsql="SELECT * FROM juicios where juicionulidad like '%$valor%' or procedimiento like '%$valor%'";
...
else 					$txtsql .= " and sub LIKE '".$nivel."%'  ";
...
$tabla = '
        ...
        <thead>
            <tr>';
				...
				$tabla .= '<th class="ancho 100 blanco">Entidad o Dependencia</th>';
				//$tabla .= '<th class="ancho100 blanco">Cuenta Pública</th>';
				$tabla .= '<th class="ancho50 blanco">Acción</th>';
				$tabla .= '<th class="ancho200 blanco">Procedimiento</th>';
				$tabla .= '<th class="ancho150 blanco">Actor </th>';
				$tabla .= '<th class="ancho100 blanco">Sala del Conocimiento</th>';
				$tabla .= '<th class="ancho250 blanco">Juicio de Nulidad</th>';				
				$tabla .= '<th class="ancho10 blanco">Fecha Not. DGR</th>';
...
while($r = mysql_fetch_array($j))
{
	...
	$tabla .= '
            <tr '.$estilo.' >';
				...
				<a href="#" title="Modificar" class="icon-1 info-tooltip" onclick=\'var cuadro2 = new mostrarCuadro(520,900,"Juicio: '.$r['nojuicio'].'",50,"cont/juicios_mod.php","juicio='. $r['id'].'")\'></a>';
