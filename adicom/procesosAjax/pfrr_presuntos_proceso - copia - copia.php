...
if($tipoFecha == 'fechaUltimaActuacion')
{
	...
	$sqltxt = "UPDATE pfrr SET detalle_edo_tramite = 28,  limite_cierre_instruccion = '".$fecha90."', usuario = '".$usuario."',fecha_edo_tramite = '".date("Y-m-d")."',hora = '".date("h:i:s")."' WHERE num_accion = '".$accion."' ";
	$sql1 = $conexion->select($sqltxt,false);
	echo print_r($_POST); 
}
//---------------------------------------------------------------------------------------------------
...
$fechaPartes = explode("-",$fecha);
		...
		$ucero="0";
		if($rr["pdr"]<10) {
		$consecutivo= $dceros.$rr["pdr"]+1;
		}
		
		if($rr["pdr"]<100 and $rr["pdr"]>10 ) {
		$consecutivo= $uceros.$rr["pdr"]+1;
		}
...
if($tipoFecha == 'fechanotires')
{
	...
	if($cambiaEdo)
	{
		...
		if ($tipo =="responsabilidad")
		{
			...
			//---- metemos historial de notificaciones ---------------------------------------------------------
			//---- metemos historial de notificaciones ---------------------------------------------------------
			// tesofe
			if($tipoNot == "tesofe"){
				$sqltxt = "INSERT pfrr_historial 
