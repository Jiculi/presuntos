...
//-- superveniente
$nosuperveniente = valorSeguro($_REQUEST['nosuperveniente']);
//--superveniente
...
if ($fondo == "fo") $sqlfondo=" and (`fondo` like '%Apoyo a Centros y Organizaciones de Educacion%' or `fondo` like '%U080%') ";
else if($fondo != "") $sqlfondo="and fondo LIKE '%".$fondo."%' ";
else $sqlfondo="";
...
//--super
if  ($nosuperveniente !="") $sqlsuperv= " AND num_accion_po LIKE '%".$nosuperveniente."%' ";
else $sqlsuperv="";
//super
...
elseif($nivel == 'S')
	$txtsql .= 'WHERE subnivel LIKE "'.$direccion.'%" '.$sqlvalor.' '.$sqledo.''.$sqlsuperv.' '.$sqlsub.' '.$sqljefe.' '.$sqlef.' '.$sqlaud.' '.$sqlcp.' '.$sqledoT.' '.$sqlpo.' '.$sqlSicsa.' '.$sqlfecha.' '.$sqlpresunto.' '.$sqlfondo.' '.$sqluaa.' '.$sqldirector.' '.$sqltipo_devolucion.' '.$sqlAmp;
//---------- ABOGADOS segun nivel ----------------------------------
else 
	$txtsql .= ' WHERE subnivel LIKE "'.$nivel.'%"  '.$sqlvalor.' '.$sqledo.''.$sqlsuperv.' '.$sqlsub.' '.$sqlef.' '.$sqljefe.' '.$sqlaud.' '.$sqlcp.' '.$sqledoT.' '.$sqlpo.' '.$sqlSicsa.' '.$sqlfecha.' '.$sqlpresunto.' '.$sqlfondo.' '.$sqluaa.' '.$sqldirector.' '.$sqltipo_devolucion.' '.$sqlAmp;
//--------------------------------------------------------------------------------------------
...
$tabla = '
        ...
        <thead>
            <tr>';
				...
				if($efCh == "true") $tabla .= '<th class="ancho200 blanco">Entidad Fiscalizada	</th>';
				$tabla .= '<th class="ancho200 blanco">Accion	</th>'; 
				if($supervenientech == "true") $tabla .= '<th class="ancho200 blanco">Superveniente	</th>';
				
				//-superveniente
				//-if($superveniente == "true") $tabla .= '<th class="ancho100 blanco">superveniente</th>';
				//-sueprveniente
				...
				$tabla .= '<th class="ancho100 blanco">Fecha de Estado de Tramite	</th>';			
