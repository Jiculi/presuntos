...
if($tipo == "notificacionEF") 
{
	...
	$sql=$conexion->select("select * from autoridades 
           ...
           where po.num_accion='".$accion."' and cargos_autoridades.tipo_de_autoridad = 'Responsable del Informe'",false);
...
echo $r["nombre"]." ".$r["apellido_paterno"]." ".$r["apellido_materno"]."|".$r["nombre_cargo"]."|".$r["entidad_fiscalizada"]."|".stripslashes(html_entity_decode($r2["referencia"]))."|".$asunto;
...
{ 
	...
	$sql=$conexion->select("select * from autoridades 
           ...
           inner join po on po.id=po_autoridad.id_accion
           inner join cargos_autoridades on autoridades.cargo=cargos_autoridades.id_cargo_autoridades
           where po.num_accion='".$accion."' and cargos_autoridades.tipo_de_autoridad = 'Titular ICC' ",false);
	$r=mysql_fetch_array ($sql);
...
if($tipo == "docu_uaa") 
{
	...
	$asunto="Se remite información a la UAA".$r['uaa']."";
	$sql=$conexion->select("SELECT * from directores_uaa 
							INNER JOIN fondos on fondos.UAA=directores_uaa.uaa
...
//------------Recordatorio Asistencia Jurídica
...
if($tipo == "recordatorio") 
{
	...
	$asunto="Recordatorio Solicitud de Opinión Técnica".$r['uaa']."";
	$sql=$conexion->select("SELECT * from directores_uaa 
							INNER JOIN fondos on fondos.UAA=directores_uaa.uaa
							where  fondos.num_accion = '".$accion1."'");
	$r=mysql_fetch_array ($sql);
