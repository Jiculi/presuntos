...
if($direccion == "DG" || $nivel == "S")
{
		...
		$sql = $conexion->select("SELECT *, o.folio AS Folio, o.id AS idFol, o.tipo AS tOficio FROM oficios o
								WHERE
								(
...
else
{
	$sql = $conexion->select("SELECT *, o.folio AS Folio, o.id AS idFol, o.tipo AS tOficio FROM oficios o
							  INNER JOIN usuarios 
							  ON o.abogado_solicitante = usuarios.usuario
							  WHERE
...
$tabla = '
	...
	while($r = mysql_fetch_array($sql))
	{
		...
		if($r['status'] == 0) $txtStatus = "<img src='images/pendientes.png'>";
		if($r['status'] == 1) $txtStatus = "";
		if($r['status'] == 2) $txtStatus = "";
		...
		if($r['tOficio'] == "opi_legal") $linkSubirArchivo = "";
		//------------------------------------------------------------------ 
