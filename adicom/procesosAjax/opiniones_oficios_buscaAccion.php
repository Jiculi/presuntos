...
if($direccion == "DG") $sql=$conexion->select("select num_accion,direccion,abogado,detalle_de_estado_de_tramite,monto_de_po_en_pesos,monto_modificado FROM opiniones where num_accion like '%$accion%'");
else $sql=$conexion->select("SELECT num_accion,direccion,abogado,detalle_de_estado_de_tramite,monto_de_po_en_pesos,monto_modificado FROM opiniones WHERE num_accion LIKE '%$accion%' AND  subnivel LIKE '".$nivel."%'  ");
...
while ($r=mysql_fetch_array ($sql))
{
	...
	$results[] = array("value"=>$r["num_accion"], "direccion"=>$r["direccion"], "turnado"=>$director, "direccion"=>$r["direccion"], "estadoTxt"=>$edoGral, "estado"=>$r["detalle_de_estado_de_tramite"], "ofiICC"=>$ofiICC, "ofiEF"=>$ofiEF, "ofiAS"=>$ofiAS, "monto"=>$monto);
}
