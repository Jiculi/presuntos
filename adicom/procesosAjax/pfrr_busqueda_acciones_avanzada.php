...
if ($edoTram == 24 ) $sqledoT= " AND detalle_edo_tramite = 24 ";
else if ($edoTram == 45 ) $sqledoT= " AND (et_impugnacion = 45 OR et_impugnacion = 46 OR et_impugnacion = 47 OR et_impugnacion = 48 )";
else if ($edoTram == 9 ) $sqledoT= " AND (detalle_edo_tramite = 23 OR detalle_edo_tramite = 24 OR detalle_edo_tramite = 25 OR detalle_edo_tramite = 26)";
else if ($edoTram == 27 ) $sqledoT= " AND (detalle_edo_tramite = 11 OR detalle_edo_tramite = 13 OR detalle_edo_tramite = 15 OR detalle_edo_tramite = 30)";
else if ($edoTram == 28 ) $sqledoT= " AND (detalle_edo_tramite = 16)";
else if ($edoTram == 29 ) $sqledoT= " AND (detalle_edo_tramite = 17 OR detalle_edo_tramite = 18 OR detalle_edo_tramite = 31 OR detalle_edo_tramite = 19 OR detalle_edo_tramite = 20 OR detalle_edo_tramite = 22 OR detalle_edo_tramite = 29)";
else if ($edoTram != 0 ) $sqledoT= " AND detalle_edo_tramite = ".$edoTram." ";
else $sqledoT= "";
...
if($inicio !="") $sqlinicio="and inicio_frr LIKE '%".$inicio."%' ";
else $sqlinicio="";
...
//--------------------------------------------------------------------------------------------
//---------- DG vemos todo -----------------------------------------
if($direccion == 'DG' || substr($nivel, 0, 3) || substr($nivel, 0, 5))
	$txtsql .= " WHERE 1=1 $sqlvalor $sqledo $sqlsub $sqlef  $sqlaud $sqlemi_res  $sqljefe $sqlnoti_res $sqllim_not_res  $sqlcp $sqllim_emi_res  $sqlcie_ins $sqledoT $sqlult_act $sqlpo $sqlacu_inicio $sqlSicsa $sqlsuperveniente $sqlfecha $sqlpresunto $sqlfondo $sqluaa $sqldirector  $sqltipo_devolucion $sqlpdr $sqlpfrr $sqltipo_abs $sqlinicio";
//---------- ASISTENTES / SECRES ven todo (anteriormente solo direccion " ELIMINADO de la consulta SQL = pfrr.subnivel LIKE '".$direccion."%' ") ----------------------------------
elseif($nivel == 'S')
	$txtsql .= " WHERE 1=1 $sqlvalor  $sqlemi_res $sqljefe $sqlnoti_res $sqllim_not_res $sqledo $sqllim_emi_res $sqlcie_ins $sqlsub $sqlult_act $sqlef  $sqlacu_inicio $sqlaud $sqlsuperveniente $sqlcp  $sqledoT $sqlpo $sqlSicsa $sqlfecha $sqlpresunto $sqlfondo $sqluaa $sqldirector  $sqltipo_devolucion $sqlpdr $sqlpfrr $sqltipo_abs $sqlinicio";
//---------- ABOGADOS segun nivel ----------------------------------
else 
	$txtsql .= " WHERE pfrr.subnivel LIKE '".$nivel."%'  $sqlvalor $sqledo   $sqlemi_res $sqljefe $sqlnoti_res $sqllim_not_res $sqlsub $sqllim_emi_res $sqlcie_ins $sqlef  $sqlaud $sqlacu_inicio $sqlcp $sqlsuperveniente $sqledoT $sqlpo $sqlSicsa $sqlfecha $sqlpresunto $sqlfondo $sqluaa $sqldirector  $sqltipo_devolucion $sqlpdr $sqlpfrr $sqltipo_abs $sqlinicio";
//--------------------------------------------------------------------------------------------
//--------------------------------------------------------------------------------------------
...
$tabla = '
        ...
        <thead>
            <tr>';
				...
				if($ampCh == "on") $tabla .= '<th class="ancho100 blanco">Ampliado</th>';
...
while($r = mysql_fetch_array($sqlT))
{
	...
	IF ($r['detalle_edo_tramite'] == "16")
			{ $edo = "Iniciado"; $edo1 = dameEstado($r['detalle_edo_tramite']); } 
	else if ($r['et_impugnacion'] == "45" || $r['et_impugnacion'] == "46" || $r['et_impugnacion'] == "47" || $r['et_impugnacion'] == "48")
		{ $edo = dameEstadoSicsa($r['et_impugnacion']); $edo1 = dameEstado($r['et_impugnacion']); }
	else 
		{ $edo = dameEstadoSicsa($r['detalle_edo_tramite']); $edo1 = dameEstado($r['detalle_edo_tramite']); }
	...
	$tabla .= '
            <tr '.$estilo.' >';
				...
				if($esCh == "on") $tabla .= '<td class="">'.$edo.'</td>';
				if($etCh == "on") $tabla .= '<td class="">'.$edo1.'</td>';
				...
				if($lim_emi_resCh == "on") $tabla .= '<td class="align="right">'.fechaNormal($r['limite_cierre_instruccion']).'</td>';
			...
			if($reports != 1)
			{
				...
				if($nivel != "S" and $nivel != "A.1.1.9" and $nivel != "B.1.1.9" and $nivel != "C.1.1.9" and $nivel != "D.1.1.9") $tabla .= '<a href="#" title="Modificar" class="icon-1 info-tooltip" onclick=\'var cuadrox1 = new mostrarCuadro(500,1250,"'.$r['pona'].'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'.$r['entidad'].'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'.dameEstado($r['detalle_edo_tramite']).'",50,"cont/pfrr_proceso.php","numAccion='.$r['pona'].'&usuario='.$_SESSION['usuario'].'&direccion='.$_SESSION['direccion'].'")\'></a>';
