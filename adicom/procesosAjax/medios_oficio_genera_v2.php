...
$sql = $conexion->select("SELECT num_accion,hora_oficio,abogado_solicitante,tipo 
                         FROM oficios WHERE num_accion LIKE '$acciones' AND hora_oficio = '$horaOficio' AND abogado_solicitante = '$userForm'   ", false);
...
if($TO == 0){
	...
	$folio = generaOficios($tipo = "medios", $fechaOficio, $horaOficio, $acciones, $presunto, $oficioRef, $remitente, $cargo, $dependencia, $asunto, $userForm, $dirForm, $tipoOficio);
	//-------------------------------------------------------------------------------
	//-------------------------------------------------------------------------------
