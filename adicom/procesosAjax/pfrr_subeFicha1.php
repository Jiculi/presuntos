...
$conexion->conectar();
//--------------------------------------------------------------
$_REQUEST['idPresuntoFicha'];
$_REQUEST['numAcccionFicha'];
//-------------------- IMAGEN ----------------------------------
if (isset($_FILES)) 
{
   $cadFile = fileData($_FILES['ficha']['name']);
   //$newNom = date("mdHis").".".$cadFile["ext"];
	
   //echo "<br>".$newNom."<br>";	
   //Recibo parámetros del archivo de upload
   
   $tipo = $_FILES['ficha']['type'];
   $nom_original =  $_FILES['ficha']['name'];
   $tamano = $_FILES['ficha']['size'];
   
	
   //-------- seleccionamos su nombre 
   $nvoNom = $_REQUEST['numAcccionFicha']."_".$_REQUEST['tipoFicha']."_".$_REQUEST['idPresuntoFicha']."_".$cadFile["name"];
  ...
  if (stripos($url, "adicom") !== false) $capeta = "/adicom/archivos/recuperaciones/";
  else $capeta = "/prueba/archivos/recuperaciones/";
		
  if(move_uploaded_file($_FILES['ficha']['tmp_name'],$_SERVER['DOCUMENT_ROOT'].$capeta.$nvoNomExt))
   {
		//$sql = $conexion->insert("INSERT INTO archivos SET num_accion = '".$_POST['accion']."', tipoDoc = '".$_POST['tipoDoc']."', oficioDoc = '".$nvoNom."', nombreDoc = '".$nvoNomExt."', contenidoDoc = '".$_FILES['ficha']['tmp_name']."' ",false);
		$sql = $conexion->insert("INSERT INTO archivos SET num_accion = '".$_POST['accion']."', tipoDoc = 'FichaDeposito', oficioDoc = '".$nvoNom."', nombreDoc = '".$nvoNomExt."' ",false);
	...
	//	$sql2 = $conexion->update("UPDATE pfrr_historial SET sicsa = '".$_POST['cralCarga']."', sicsaRecepcion = '".date("Y-m-d")."' , atendido = 1 WHERE oficio = '".$_POST['oficio']."' AND num_accion = '".$_POST['accion']."' ",false);
		...
		echo "<center><span style='color:green; font-weight:bold'>Se subio el archivo correctamente el archivo $nvoNomExt </span></center>"; 
   }
   else
   {
		echo "<center><span style='color:red; font-weight:bold'>No se subio ficha </span></center>"; 
   }
}
//---------------------------------------------------- SUBE IMAGEN ----------------------------------------------------------
