...
{
	if($direccion == "DG")
		$sql = $conexion->select("SELECT num_accion FROM pfrr WHERE num_accion LIKE '%".$accion."%' ",false);
	else 
		$sql = $conexion->select("SELECT num_accion FROM pfrr WHERE direccion = '".$direccion."' AND num_accion LIKE '%".$accion."%' ",false);
}
//else 
