...
$sqlchn = $conexion->select("SELECT * FROM medio_historial WHERE Procedimiento = '".$procedimiento."' and Tipo = 'rr_notifica' ",false);
$rre = mysql_fetch_array($sqlchn);
...
if($rre['Fecha_Acuse'] != "" and $rre['Fecha_Acuse'] != "0000-00-00" and $ntr == 39)	
{ 
$query = "UPDATE actores_recurso 
...
if($rre['Fecha_Acuse'] != "" and $rre['Fecha_Acuse'] != "0000-00-00" and $ntr == 391)
{
		$query = "UPDATE actores_recurso 
...
$sqlchs = $conexion->select("SELECT * FROM medio_historial WHERE Procedimiento = '".$procedimiento."' and (Tipo = 'rr_not_acuerdo_s' or Tipo = 'rr_not_des_s') ",false);
$rres = mysql_fetch_array($sqlchs);
...
if($edotram == 39)
{
	...
	if($rres['Fecha_Acuse'] != "" and $rres['Fecha_Acuse'] != "0000-00-00"){
	$query = "UPDATE actores_recurso 
			  SET detalle_edo_tramite= 36,
				  usuario='".$usuario."'
			  WHERE recurso_reconsideracion='".$procedimiento."' ";
...
/*//--------------- formulario estado 43 NOTIFICACIÓN D ELA RESOLUCIÓN
//--------------- formulario estado 43 NOTIFICACIÓN D ELA RESOLUCIÓN
if($_REQUEST['tipoForm'] == "rr_NotRes")
{
	$query = "INSERT INTO medios_historial SET
				num_accion = '".$_REQUEST['numAccion']."',
				tipo = '".$_REQUEST['tipoForm']."',
				notificacion = '".$_REQUEST['tipoNotRes']."',
				oficio = '',
				oficioRecepcion = '".fechaMysql($_REQUEST['fecha_not_res'])."',
				oficioAcuse = '',
				estadoTramite = 43,
...
////////////////////////////////////////////////////////notificaciones presunto / sat
/*
if($_REQUEST['tipoForm'] == "rr_notpre" || $_REQUEST['tipoForm'] == "rr_notsat")
{
$numAccion = valorSeguro($_POST['accion_emr']);
$procedimiento = valorSeguro($_POST['procedimiento_emr']);
$actor = valorSeguro($_POST['id_actor_emr']);
$usuario = valorSeguro ($_POST['us_emr']);
$fecha_acuseo = valorSeguro($_POST['notificacionresoT']);
$fecha_acuseo2 = valorSeguro($_POST['notificacionresoT2']);
$rr_reqRecRec = valorSeguro($_POST['tipoForm']);
$valtipo = valorSeguro($_POST['tipo_emi_res']);

if($rr_reqRecRec == "rr_notpre")
{	
	$query = "INSERT INTO medio_historial 
			  SET accion = '".$numAccion."',
				Tipo = '".$rr_reqRecRec."',
