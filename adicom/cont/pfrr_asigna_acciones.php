...
if($direccion == "DG")
	$sql = $conexion->select("SELECT * FROM usuarios WHERE LENGTH(nivel) >= 1 AND status != '0' AND (nivel != 'DG' and nivel != 'S') order by nivel",false);
if($nivel == "S")
	$sql = $conexion->select("SELECT * FROM usuarios WHERE LENGTH(nivel) >= 1 AND nivel LIKE '".$nivel."%' AND status != '0' AND (nivel != 'DG' and nivel != 'S') order by nivel",false);
if($tamNiv == 1 && $nivel != "S")
	$sql = $conexion->select("SELECT * FROM usuarios WHERE LENGTH(nivel) >= 3 AND nivel LIKE '".$nivel."%' AND status != '0' AND (nivel != 'DG' and nivel != 'S') order by nivel",false);
if($tamNiv == 3)
	$sql = $conexion->select("SELECT * FROM usuarios WHERE LENGTH(nivel) >= 5 AND nivel LIKE '".$nivel."%' AND status != '0' AND (nivel != 'DG' and nivel != 'S') order by nivel",false);

if($tamNiv == 5)
	$sql = $conexion->select("SELECT * FROM usuarios WHERE LENGTH(nivel) >= 7 AND nivel LIKE '".$nivel."%' AND status != '0' AND (nivel != 'DG' and nivel != 'S') order by nivel",false);
...
function asignarAccion()
{
	if(comprobarForm('asignaForm'))
	{
		...
		({
			...
			type: "POST",
			url: "procesosAjax/pfrr_asigna_acciones.php",
