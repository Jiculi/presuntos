...
<div class="contPasos">
    ...
    <div id='resPasos' class='divPresuntos redonda10'>
        ...
        <div id="p1" class="todosContPasos pInactivo">
		   ...
		   $sql=$conexion->select("select * from autoridades 
           ...
           where po.num_accion='".$accion."' ",false);
            ...
            while($r = mysql_fetch_array($sql))
            {
				...
				if($r['id_cargo_autoridades'] >= 2 or $r['id_cargo_autoridades'] <= 7 or $r['id_cargo_autoridades'] == 31) $TP = "Titular de la Entidad Fiscalizada";
				if($r['id_cargo_autoridades'] == 1 or $r['id_cargo_autoridades'] >= 8 and $r['id_cargo_autoridades'] <> 31) $TP = "Titular de Instancia de Control Competente";
                ...
                $tabla .= '
                        <tr '.$estilo.' >
                            ...
                            <td class="contratista">'.$r['nombre'].' '.$r['apellido_paterno'].' '.$r['apellido_materno'].'</td>
                            <td class="cargo">'.$r['nombre_cargo'].'</td>
							<td class="cargo">'. stripslashes($r['entidad_fiscalizada']).'</td>
                           						';
				$wtabla .= '<a href="#" title="'.$title.'" class="icon-1 info-tooltip" onclick=\'var cuadro5 = new mostrarCuadro3(450,600,"'.$title.'",100,"cont/po_autoridad_mod.php","idPresunto='.$r['creacion'].'&numAccion='.$r['num_accion'].'&usuario='.$_REQUEST['usuario'].'&direccion='.$_REQUEST['direccion'].'")\'></a>';
                $tabla .= '</td></tr>';
            }
              ...
              <table width="90%" align="center" class="tablaPasos">
               <tr valign="baseline">
                ...
                <tr >
                  ...
                  <input type="button" onclick="guardaAutoridad()"  class="submit-login"  value="Agregar" name="Actualizar registro" id="Actualizar registro">
                  </center>
                  </td>
