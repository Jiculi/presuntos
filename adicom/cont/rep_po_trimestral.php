...
<table width="100%">
    <tr>
        ...
        <?php if($_SESSION['direccion'] == "DG" || $_SESSION['nivel'] == "S") { ?>
                <form action="reportes/po_trimestral_cp2012.excel.php" method = "POST" >
                    <input type="hidden" name="export" value= <?php //echo urlencode($data) ?> />
                    <input type="hidden" name="nombre_archivo" value="Reporte de Notificaciones"/>
                    <input type = "submit" class="submit_line" value = "Descargar en Excel" class="links">
				</form>
        <?php } ?>
        <iframe width="100%" height="400" frameborder="0" src="reportes/po_trimestral_cp2012.pdf.php"></iframe>
        </td>
