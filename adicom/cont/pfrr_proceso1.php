                     ...
                     <center>
                         <table class="tabladtns" width="90%" align="center">
                          ...
                          <tr valign="baseline">
                           ...
                           <tr valign="baseline">
                              <td valign="top" width="25%"  class="">
                                <input type="checkbox" name="solUAAsolventacion" id="solUAAsolventacion">
                                <label for="solUAAsolventacion"> A solicitud de la UAA, Por posible solventación</label>
                              </td>
                           </tr> 
                         </table>
 ...
 <!-- ----------------------------- DIV ACUERDO DE INICIO --------------------------------- -->
    ...
    <div id="p9" class="todosContPasos pInactivo">
        ...
        <?php
		  ...
		  $ofSQL = "SELECT *,oficios.folio as OficioFolio FROM oficios_contenido INNER JOIN oficios ON oficios_contenido.folio = oficios.folio WHERE oficios.tipo = 'opinion_UAA_PFRR' AND oficios_contenido.num_accion  = '".$accion."' AND status <> 0 LIMIT 1";
        ...
        $$( "#fechaAcuseUAA" ).datepicker({
         // dateFormat: formatoFecha,
              ...
              numberOfMonths: 1,
              showAnim:'slideDown',
              minDate: '<?php echo fechaNormal($rUA['fecha_oficio']) ?>',
              beforeShowDay: noLaborales
			  /*
              onClose: function( selectedDate ) 
              {
    ...
    <div id="p8" class="todosContPasos pInactivo">
        ...
        <?php 
			...
			$sql2=$conexion->select("SELECT * FROM pfrr_presuntos_audiencias WHERE num_accion = '".$r["num_accion"]."' AND (tipo <> 'titularICC' AND tipo <> 'responsableInforme' AND tipo <> 'titularTESOFE') ORDER BY cont ");
        ...
        <div style="float:right"><img src="images/help.png" /></div>
             ...
             <?php 
			  ...
			  $tipo = mysql_fetch_array($sql);
				...
				//-------------------------------------------------------------------------------------------
				//-------------------------------------------------------------------------------------------
				  $sqlres=$conexion->select("SELECT resolucion,tipo_resolucion FROM pfrr WHERE num_accion = '".$accion."' "); 
                        ...
                        <table class='feDif' width='80%' align='center'>
                           ...
                           <!--
                            ...
                            <tr>
                                ...
                                <?php
								$sqlPre = $conexion->select("SELECT * FROM pfrr_presuntos_audiencias WHERE num_accion = '".$accion."'  AND (tipo <> 'titularICC' AND tipo <> 'responsableInforme' AND tipo <> 'titularTESOFE') ",false);
