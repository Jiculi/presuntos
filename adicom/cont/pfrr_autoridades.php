...
$sql3 = $conexion->select("SELECT * FROM pfrr_presuntos_audiencias WHERE num_accion = '".$accion."' AND tipo = 'titularICC' ",false);
$p = mysql_fetch_array($sql3);
$tICC = $total = mysql_num_rows($sql3);
//------------------------------------------------------------------------------
$sql3 = $conexion->select("SELECT * FROM pfrr_presuntos_audiencias WHERE num_accion = '".$accion."' AND tipo = 'responsableInforme' ",false);
$p = mysql_fetch_array($sql3);
$tEF = $total = mysql_num_rows($sql3);
//------------------------------------------------------------------------------
$sql3 = $conexion->select("SELECT * FROM pfrr_presuntos_audiencias WHERE num_accion = '".$accion."' AND tipo = 'titularTESOFE' ",false);
$p = mysql_fetch_array($sql3);
$tTE= $total = mysql_num_rows($sql3);
//------------------------------------------------------------------------------
?>  
...
<div class="contPasos">
    <div class="encPasos">
       ...
       -->
       <div id='paso1' <?php echo $onclickP1 ?> class="todosPasos <?php echo $txtPaso1 ?> <?php echo $acceso1 ?> pasos"><!--<div id='np1' class="todosNP noPaso redonda10 <?php echo $numPaso1 ?>">1</div>--> AUTORIDADES  </div>
       <?php //if($tEF == 0 || $tICC == 0) { ?>
    ...
    <div id='resPasos' class='divPresuntos redonda10'>
        ...
        <div id="p1" class="todosContPasos pInactivo">
            ...
            <h3 class="poTitulosPasos">Autoridades  </h3>
			...
			';       
            $sql = $conexion->select("SELECT * FROM pfrr_presuntos_audiencias WHERE num_accion = '".$accion."' AND (tipo = 'responsableInforme' OR  tipo = 'titularICC' OR  tipo = 'titularTESOFE') ",false);
			$total = mysql_num_rows($sql);
            while($r = mysql_fetch_array($sql))
            {
				...
				if($r['tipo'] == "responsableInforme") $TP = "Titular de la Entidad Fiscalizada";
				if($r['tipo'] == "titularICC") $TP = "Titular de Instancia de Control Competente";
				if($r['tipo'] == "titularTESOFE") $TP = "Titular del SAT";
				if($r['tipo'] == "presuntoResponsable") $TP = "Presunto Responsable";
				if($r['tipo'] == "proveedor") $TP = "Proveedor";
				if($r['tipo'] == "contratista") $TP = "Contratista";
                ...
                $tabla .= '
                        <tr '.$estilo.' >
							...
							';
				$tabla .= '<a href="#" title="'.$title.'" class="icon-1 info-tooltip" onclick=\'var cuadro5 = new mostrarCuadro3(450,600,"Modificar",100,"cont/pfrr_autoridad_mod.php","idPresunto='.$r['cont'].'&numAccion='.$r['num_accion'].'&usuario='.$_REQUEST['usuario'].'&direccion='.$_REQUEST['direccion'].'")\'></a>';
                $tabla .= '</td></tr>';
            }
			if($total == 0) $tabla .= "<tr><td colspan='5'><center><h3 style=' background:#fff; padding:30px; margin:20px'>Esta Accion no cuenta aun con Autoridades</h3></center></td></tr>";
              ...
              <table width="90%" align="center" class="tablaPasos">
               <tr valign="baseline">
                  ...
                  <select  class="redonda5" id="new_tipoAutoridad"  name="new_tipoAutoridad" >
                    ...
                    <?php if($tEF <= 6) {?> <option value="responsableInforme">Titular de la Entidad Fiscalizada</option> <?php } ?>
                    <?php if($tICC <= 6) {?> <option value="titularICC">Titular Instancia de Control Competente</option> <?php } ?>
                    <?php if($tTE <= 6) {?> <option value="titularTESOFE">Titular del SAT</option> <?php } ?>
                  </select>
                ...
                <tr >
				  ...
				  <td><input  type="text"  class="redonda5" id="new_dependencia" name="new_dependencia" value="<?php echo $entidadFis ?>" size="70"></td>                  </td>
                </tr>

                  <td class="center" colspan="3">
                  <center><br /><br />
                  <input name="creacion" type="hidden" id="new_creacion" value="">
                  <input type="button" onclick="guardaAutoridadPFRR()"  class="submit-login"  value="Agregar" name="Actualizar registro" id="Actualizar registro">
                  </center>
                  </td>
