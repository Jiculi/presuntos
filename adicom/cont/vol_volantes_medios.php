...
function agregarCampos(estado,edoTxt,turnado,accion,ref,asunto,remitente,cargo,dependencia,oficio,ofiFec,ofiFecAcu,cral,cralFe,cralFecAcu,dt,ofConcluida,feConcluida)
{
	var igual = 0;
...
function insertaAccion(){
	if(comprobarForm('volanteForm')){
		...
		agregarCampos(estado,edoTxt,turnado,accion,ref,asunto,remitente,cargo,dependencia,oficio,ofiFec,ofiFecAcu,cral,cralFe,cralFecAcu,dt,ofConcluida,feConcluida)
...
function generarVolante()
{
	...
	if(comprobarForm('volanteForm') && ($$("#totalNoAcciones").val() != 0 || $$("#totalNoAcciones").val() != "")){
		...
		({
			...
			type: "POST",
			url: "procesosAjax/vol_genera_volante_po.php",
