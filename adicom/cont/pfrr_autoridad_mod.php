...
$conexion->conectar();
//------------------------------------------------------------------------------
$sql = $conexion->select("SELECT * FROM pfrr_presuntos_audiencias WHERE cont = ".$_REQUEST['idPresunto']." ",true);
...
<form action="<?php echo $editFormAction; ?>" method="POST" name="autoridad_MOD" id="form1">
  <table width="90%" align="center" class="tablaPasos ">
    ...
    <tr valign="baseline">
      ...
      <td ><input  type="text"  class="redonda5" id="servidor_contratista"  name="servidor_contratista" value="<?php echo $r['nombre']; ?>" size="70" <?php echo  $soloLectura ?>></td>
    </tr>
    <tr>
      <td class="etiquetaPo">Cargo:</td>
      <td><input  type="text"  class="redonda5" id="cargo_servidor" name="cargo_servidor" value="<?php echo $r['cargo']; ?>" size="70" <?php echo  $soloLectura ?>></td>
    </tr>
    <tr >
    ...
    <tr >
      ...
      <?php if($rPo['detalle_de_estado_de_tramite'] < 6 ) {?>
      	<input type="button"  class="submit-login"  value="Actualizar Autoridad" onclick="actualizaAutoridad('<?php echo $_REQUEST['numAccion'] ?>','<?php echo $_REQUEST['usuario'] ?>','<?php echo $_REQUEST['direccion'] ?>')">
      <?php } ?>
      </center>
      </td>
