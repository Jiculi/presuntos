...
if (isset($_FILES)) 
{
  ...
  if (stripos($url, "adicom") !== false) $capeta = "/dgr/archivos/";
  ...
  if(move_uploaded_file($_FILES['adjunto']['tmp_name'],$_SERVER['DOCUMENT_ROOT'].$capeta.$nvoNomExt))
   {
	   // cualquier documento se inserta en la tabla archivos
		$query1 = "INSERT INTO archivos SET num_accion = '".$_POST['accion']."',oficio = '".$oficio."', tipoDoc = '".$_POST['tipoDoc']."', oficioDoc = '".$nvoNom."', nombreDoc = '".$nvoNomExt."' ";
		$sql1 = $conexion->insert($query1,false);
		
		if($_POST['tipoDoc'] == "sicsa"){
			$query2 = "UPDATE po_historial SET sicsa = '".$_POST['nomDoc']."', sicsaRecepcion = '".date("Y-m-d")."' , atendido = 1 WHERE oficio = '".$oficio."' AND num_accion = '".$_POST['accion']."'  ";
			$sql2 = $conexion->update($query2,false);
		}
		
		if($_POST['tipoDoc'] == "pfrr"){
			$query2 = "UPDATE pfrr SET superveniente = '".$_POST['superveniente']."',validacion = 1 WHERE num_accion = '".$_POST['accion']."'  ";
			$sql2 = $conexion->update($query2,false);
		}
		
		if($_POST['tipoDoc'] != "pfrr"){
			$query3 = "UPDATE oficios_contenido SET atendido = 1 WHERE folio = '".$oficio."' AND num_accion = '".$_POST['accion']."' ";
			$sql3 = $conexion->update($query3,false);
		}
