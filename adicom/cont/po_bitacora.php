...
{
	if(tipo == 1) new mostrarCuadro2(600,800,"Subir Archivos",10,"cont/po_subirArchivos.php","accion=<?php echo $_REQUEST['numAccion'] ?>");//alto,ancho,titulo,top,pagina,datos
	if(tipo == 2) new mostrarCuadro2(600,800,"Subir Archivos",10,"cont/po_subirOtrosArchivos.php","accion=<?php echo $_REQUEST['numAccion'] ?>");//alto,ancho,titulo,top,pagina,datos
	if(tipo == 3) new mostrarCuadro2(600,800,"Subir Archivos",10,"cont/po_subirOtrosArchivosGen.php","accion=<?php echo $_REQUEST['numAccion'] ?>");//alto,ancho,titulo,top,pagina,datos
	if(tipo == 4) new mostrarCuadro2(600,800,"Capturar Observaciones al Estado de Trámite",10,"cont/po_oet.php","accion=<?php echo $_REQUEST['numAccion'] ?>");//alto,ancho,titulo,top,pagina,datos
	if(tipo == 5) new mostrarCuadro2(600,800,"Capturar Observaciones a la Prescripción Resarcitoria",10,"cont/po_opr.php","accion=<?php echo $_REQUEST['numAccion'] ?>");//alto,ancho,titulo,top,pagina,datos
 //$$("#cuadroRes2").html(texto);	
}
...
<div class="encVol">
	...
	<div id='paso5' onclick="muestraPestanaBit(5)" class="todosPasos pasosActivo pasos"> OBSERV. PR </div>
...
<?php  
//------------------- datos de PO --------- ---------------------------------   
$sql1 = $conexion->select("SELECT num_accion,numero_de_pliego FROM po WHERE num_accion = '".$accion."' ",false);
$po = mysql_fetch_array($sql1);
$total1 = mysql_num_rows($sql1);
//------------------- datos de NOTIFICACION --------- -----------------------    
$sql2 = $conexion->select("SELECT * FROM po_historial WHERE num_accion = '".$accion."' AND status = 1 AND oficioNotEntidad <> '' order by fechaSistema desc limit 1 ",false);
$not = mysql_fetch_array($sql2);
$total2 = mysql_num_rows($sql2);
//------------------- datos de PO HISTORIAL ---------------------------------  
$sql = $conexion->select("SELECT * FROM po_historial WHERE num_accion = '".$accion."' AND po_historial.status = 1 order by fechaSistema desc, horaSistema desc, estadoTramite desc ",false);
...
while($r = mysql_fetch_array($sql))
{
	...
	//--------------- textos de Aspectos legales --------------------------------
	if($r['estadoTramite'] == 2) $txtTipo = '<span class="fechaPO"></span>'.'Mediante oficio número '.cadenaNormalOficio($r['oficio']).' de fecha '.fechaNormal($r['oficioRecepcion']).',	recibido el día '.fechaNormal($r['oficioAcuse']).', la '.$rX['UAA'].' remitió a esta DGR, un PPO junto con su ET para su Asistencia Jurídica.		Se recibió junto con el CRAL del SICSA número '.utf8_encode($r['sicsa']).' de fecha '.fechaNormal($r['sicsaRecepcion']).' y se generó el volante interno número '.$r['volante'].'.';
	if($r['estadoTramite'] == 3	|| $r['estadoTramite'] == 4) $txtTipo = '<span class="fechaPO"></span>'.'Mediante oficio número '.cadenaNormalOficio($r['oficio']).' de fecha '.fechaNormal($r['oficioRecepcion']).' y entregado el dia '.fechaNormal($r['oficioAcuse']).',  esta DGR brindó Asistencia Jurídica y, en cumplimiento a la fracción VII de los “Lineamientos para la Formulación y Notificación del Pliego de Observaciones”, remitió a la '.$rX['UAA'].', PPO y ET con el análisis de los requisitos para la formulación del PO y se emitieron comentarios y/o sugerencias en torno a su contenido; a fin de que valorara su implementación y lo devolviera para su notificación correspondiente. Se emitio el CRAL  del SICSA número '.$r['sicsa'].' el dia '.fechaNormal($r['sicsaRecepcion']).'.';
	if($r['estadoTramite'] == 5) $txtTipo = '<span class="fechaPO"></span>'.'Mediante tarjeta número '.cadenaNormalOficio($r['oficio']).' de fecha '.fechaNormal($r['oficioRecepcion']).', recibido el día '.fechaNormal($r['oficioAcuse']).', la oficina del AEGF remitió a esta DGR el PO firmado. Se inició trámite de certificación y el proceso para su notificación correspondientes. Se generó el volante interno número '.$r['volante'].'.';
	if($r['estadoTramite'] == 6) $txtTipo = '<span class="fechaPO"></span>'.'Mediante oficios número '.$r['oficioNotEntidad'].' y '.$r['oficioNotOIC'].' de fecha '.fechaNormal($r['fechaNotEntidad']).'  y con fechas de acuse '.fechaNormal($r['acuseNotEntidad']).' y '.fechaNormal($r['acuseNotOIC']).', se Notificó el PO '.$po['numero_de_pliego'].', a la EF y a la ICC, respectivamente.';
	if($r['estadoTramite'] == 7) $txtTipo = '<span class="fechaPO"></span>'.'Mediante oficio número '.cadenaNormalOficio($r['oficio']).' de fecha '.fechaNormal($r['oficioRecepcion']).' y con fecha de entrega '.fechaNormal($r['oficioAcuse']).', se remitió a la '.$rX['UAA'].' los acuses originales del '.$po['numero_de_pliego'].' y oficios '.$not['oficioNotEntidad'].' y '.$not['oficioNotOIC'].' con fecha de acuse del día '.fechaNormal($not['acuseNotEntidad']).', mediante los cuales se notificó tanto al titular de la EF como al titular de la ICC; y se invitó a considerar que el plazo de 30 días hábiles vence el día  '.fechaNormal(sumaDiasNoInhabiles($not['acuseNotEntidad'],30)).'. ';
	if($r['estadoTramite'] == 8) $txtTipo = '<span class="fechaPO"></span>'.'Mediante copia del oficio número '.cadenaNormalOficio($r['oficio']).' de fecha '.fechaNormal($r['oficioRecepcion']).', recibido el dia '.fechaNormal($r['oficioAcuse']).' la '.$rX['UAA'].' hace de nuestro conocimiento que se notificó a la EF la Baja por Conclusión, previa a la emisión del PO. Se generó el volante interno número '.$r['volante'].'. ';
	if($r['estadoTramite'] == 9) $txtTipo = '<span class="fechaPO"></span>'.'Mediante copia del oficio número '.cadenaNormalOficio($r['oficio']).' de fecha '.fechaNormal($r['oficioRecepcion']).', recibido el dia '.fechaNormal($r['oficioAcuse']).' la '.$rX['UAA'].' hace de nuestro conocimiento que se notificó a la EF la Solventación del PO numero '.$po['numero_de_pliego'].'. Se generó el volante interno número '.$r['volante'].'. ';
	if($r['estadoTramite'] == 10)$txtTipo = '<span class="fechaPO"></span>'.'Mediante oficio número '.cadenaNormalOficio($r['oficio']).' de fecha '.fechaNormal($r['oficioRecepcion']).', recibido el día '.fechaNormal($r['oficioAcuse']).', la '.$rX['UAA'].' remitió a esta DGR el DTNS numero  '.$r['num_DT'].' correspondiente al PO número '.$po['numero_de_pliego'].', acompañado de su respectivo ET y proyecto de DT  para que, de proceder, se inicie el PFRR. Se recibió junto con el CRAL del SICSA número '.$r['sicsa'].'. De fecha '.fechaNormal($r['sicsaRecepcion']).' y se generó el volante interno número '.$r['volante'].'.';
...
?>
   ...
   <?php
    $sql = $conexion->select("SELECT * FROM po_histo_ant WHERE num_accion = '".$accion."' ",false);
    $total = mysql_num_rows($sql);
    $tabla = '
...
<!--- ---------------------------------------------------------------------------------------------- -->
<!--- ---------------------------------------------------------------------------------------------- -->
<div id='p5' class="contOficios redonda10 todosContPasos" style="display:none">

<?php
//if($direccion == "DG")
	echo " <a href='#' onclick='subirArchivos(5)'> Capturar Observaciones <img src='images/Upload.png' /> </a> ";
?>

    <h3 class="poTitulosPasos">Observaciones a la Prescripción Resarcitoria</h3>
    
   <?php
    $sql = $conexion->select("SELECT * FROM po_histo_opr WHERE num_accion = '".$accion."' ",false);
    $total = mysql_num_rows($sql);
    $tabla = '
            <table border="0" align="center" cellpadding="0" cellspacing="0" id="product-table" >
                <tr>
            ...
            while($r = mysql_fetch_array($sql))
            {
                ...
                $tabla .= '
                        <tr '.$estilo.' >
                            ...
                            <td width="804" ><h4>'.$r['datos'].'</h4></td>
							<td width="100" ><h4>' .fechaNormal($r['fecha']).'</h4></td>
