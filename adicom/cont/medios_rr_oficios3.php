...
function comprobarFormulario(form,estado,volantes,oficios,cral,SolventacionBaja)
{
	...
	//------------OFICIO DE PREVENCION 
	    var numVcs = 0;
	if( $$('#tipoOficio').val() == 'rr_prevención')
	{
		$$(".camposInputEstados").each(function() {
			numVcs++;
    ...
    //------------
	//-----------OFICIO DE NOTIFICACION DEL ACUERDO
	
	//----------------------------------------------
	
	//----------OFICIO DE NOTIFICACION AL SAT
	//-----------------------------------------------
	
	//-----------OFICIO DE NOTIFICACION AL RECUERRENTE
...
var accionesNum = 0;
//--------------------------------------
function agregarCampos(valor,estado,estadoTxt,oficioEF,oficioICC,oficioAS,monto,idPresunto,accion,presunto)
{
	var igual = 0;
	...
	if(igual == 0)
	{
		...
		campo += '<input type="hidden" size="80" class="redonda5 camposInputAcciones" id="nomPresunto[]"  name="nomPresunto[]" value="'+presunto+'"  readonly/>';
		campo += '<input type="hidden" size="35" class="redonda5 camposInputAcciones" id="idPresunto[]"  name="idPresunto[]" value="'+idPresunto+'"  readonly/>';
		campo += '<input type="hidden" size="35" class="redonda5 camposInputAcciones" id="accion[]"  name="accion[]" value="'+accion+'"  readonly/>';
...
$$(function() 
{
	 ...
	 $$("#txtRecurrente").autocomplete({
	  ...
	  select: function( event, ui ) {  
			...
			if(ui.item.estado == 37) {
				 $$("#tipoOficio").append(' <option class="recurso" value="45">Solicitud de Información/Documentación a EF</option> ');
				 //$$("#oficioRef").val("");
			...
			//-------------------------------------------------
			agregarCampos(ui.item.value,ui.item.estado,ui.item.estadoTxt,ui.item.ofiEF,ui.item.ofiICC,ui.item.ofiAS,ui.item.monto,ui.item.idPresunto,ui.item.accion,ui.item.presunto);
	  },
        	...
        	<td width="30%">
            	...
            	<table  align="center" width="100%" border="0" class="tablaPasos tablaVol">
                    ...
                    <td style="position:relative">
                      <?php if($_REQUEST['actor'] != "") echo "<div class='mensajeInfo' id='mensajeRec' style='position:absolute; width:180px; left:230px; top:-10px'> <img src='images/cross.png' style='float:right; padding:5px; cursor:pointer' onclick=\"$$('#mensajeRec').fadeOut()\" /> Pulse la tecla &darr; para seleccioner al recurrente. </div>"; ?>
                      <input type="text" name="txtRecurrente" id="txtRecurrente" class="redonda5" size="35"  style="float:left;" value="<?php echo $_REQUEST['num_accion'] ?>"> 
                ...
                <?php //echo $_SESSION['nivel']; ?>
                  <tr>
                    ...
                    <td>
                      <select name="tipoOficio" id="tipoOficio" class="redonda5" onchange="">
                      		<option class="" value="">Seleccione el tipo de oficio...</option>
                            <option value="prevension">Oficio de Prevensión</option>
                            <option value="admision">Notificación del Acuerdo</option>
                            <option value="desechamiento">Notificación de la Resolución</option>
