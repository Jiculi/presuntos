...
function muestraPestanaVol(divId)
{
	...
	if(divId == 2)
	{
		$$('#volLista').html('<center><img src="images/load_grande.gif" style="margin:100px 0"></center>');
  		$$("#volLista").load("procesosAjax/medios_oficio_busca.php",{direccion:"<?php echo $_SESSION['direccion'] ?>",usuario:"<?php echo $_SESSION['usuario'] ?>",nivel:"<?php echo $_SESSION['nivel'] ?>"});
	} 
	if(divId == 3)
	{
	...
	$$("#text").keyup(function() {
		$$.ajax
			({
				...
				type: "POST",
				url: "procesosAjax/medios_oficio_busca.php",
				data: {
						texto:$$('#text').val(),
