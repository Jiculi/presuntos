			...
			$$.ajax({
				...
				success: function(data) 
					{ 	
						...
						$$("#RS_cantidad").val('');
						cerrarCuadro2();
						mostrarCuadro(600,900,"Informacion de la Acción",20,"cont/opiniones_informacion.php","numAccion="+accion)
					}
			});
		}//end confirm 
	}
...
<?php 
	...
	<link href="file://///adicom/adicom/dgr/css/estilos_opiniones.css" rel="stylesheet" type="text/css" />
<a href="#" title="Asignar Acción" class="" onClick=" new mostrarCuadro2(250,800,'Asignar Acción <?php echo $_REQUEST['numAccion'] ?>',100,'cont/opiniones_asigna_acciones.php','accion=<?php echo $_REQUEST['numAccion'] ?>&direccion=<?php echo $_REQUEST['direccion'] ?>&usuario=<?php echo $_REQUEST['usuario'] ?>&nivel=<?php echo $_REQUEST['nivel'] ?>') "> 
...
$sqlf= $conexion->select("SELECT * from fondos where num_accion='".$accion."'",false);
$rf= mysql_fetch_array($sqlf);

$consulta=$conexion->select("SELECT num_accion_po from opiniones where num_accion='".$accion."'");
$accion_po=$consulta;
$num_accion_po=mysql_fetch_array($accion_po);
$rq=$num_accion_po['num_accion_po'];

$consulta1=$conexion->select("SELECT fondo from opiniones where num_accion='".$accion."'");
$tipoFondo=mysql_fetch_array($consulta1);
$fondo=$tipoFondo['fondo'];
...
$sqlmonto= $conexion->select("SELECT monto_de_po_en_pesos FROM opiniones WHERE num_accion='".$accion."'");
$m = mysql_fetch_array($sqlmonto);
$monto = $m['monto_de_po_en_pesos'];
...
?>
        <table align="center" class='tablaInfo' width="100%">
          ...
          <tr>
            <td width="24%"><p class="etiquetaInfo redonda3">Solicitud de Aclaración</p></td>
       		<td width="26%"><p class="txtInfo"><?php echo $accion ?></p></td>
            <td width="21%"><p class="etiquetaInfo redonda3">Clave de Acción</p></td>
            <td width="29%"><span class="txtInfo"><?php echo $rq ?></span></td>
          </tr>
          <tr>
          ...
          <tr>
           <td><p class="etiquetaInfo redonda3">Director</p></td>
            ...
            <td><p class="txtInfo"> <?php echo $r['numero_de_pliego']; ?></p></td>   
          ...
          <td><p class="etiquetaInfo redonda3">Fecha de irregularidad General</p></td>
           ...
           <td><p class="etiquetaInfo redonda3">UAA</p></td>
            ...
            <td><p class="txtInfo"><?php echo $fondo; ?></p></td>
            </tr>
            
            <tr>       <td><div class="etiquetaInfo redonda3"> Monto </div></td>
            <td><p class="txtInfo">
			<?php echo number_format($monto)?></p></td></tr>
...
<!-- ------------------------------------------------------------------------------------------------------------------------------ --> 
    ...
    <tr>
            ...
            <br />
              <br />  
            <td class="tablaInfo2"> <p align="center" class="etiquetaInfo2">Irregularidad General</p></td>
          </tr>
          <tr>	
            <td><p style= "text-align:justify; padding:	"><?php 
          ...
          <td align="center">
            	...
            	<form method="post" name="formM" action="<?php echo $editFormAction; ?>" onSubmit="return false">
                  <table width="71%" height="" align="center" class="center RS_tabForm">
