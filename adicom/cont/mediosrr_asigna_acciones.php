...
if($direccion == "DG")
	$sql = $conexion->select("SELECT * FROM usuarios WHERE LENGTH(nivel) = 7 order by nombre",false);
if($nivel == "S")
	$sql = $conexion->select("SELECT * FROM usuarios WHERE LENGTH(nivel) = 7 AND (nivel LIKE '".$direccion."%' AND nivel NOT LIKE '%BECARIO%' ) order by nombre",false);
if($tamNiv == 1 && $nivel != "S")
	$sql = $conexion->select("SELECT * FROM usuarios WHERE LENGTH(nivel) = 7 AND (nivel LIKE '".$nivel."%' AND nivel NOT LIKE '%BECARIO%' ) order by nombre",false);
if($tamNiv == 3)
	$sql = $conexion->select("SELECT * FROM usuarios WHERE LENGTH(nivel) = 7 AND (nivel LIKE '".$nivel."%' AND nivel NOT LIKE '%BECARIO%' ) order by nombre",false);
	
$totalA = mysql_num_rows($sql);

$sqlx = $conexion->select("SELECT abogado FROM actores_recurso WHERE recurso_reconsideracion like '%".$accion."%' ",false);
$abogado = mysql_fetch_array($sqlx,MYSQL_ASSOC);

//$abogado = dameUsuario($abogado['usuario']);
//print_r($abogado);

//$usuario = $abogado['usuario'];
$nombre = $abogado['abogado'];
...
function asignarAccion()
{
	if(comprobarForm('asignaForm'))
	{
		...
		({
			...
			type: "POST",
			url: "procesosAjax/medios_rr_asigna_acciones.php",
        ...
        <tr>
        	<td width="50%">
            	...
            	<table  align="center" width="100%" border="0" class="tablaPasos tablaVol">
                  <tr>
                    <td class="etiquetaPo">  <p>Presunto:</p></td>
                    <td>
                      <input type="text" name="presuntoA" id="presuntoA" size="35" class="redonda5 camposEditar" value="<?php echo $presunto ?>" readonly="readonly">
                    </td>
                  </tr>
            	<table  align="center" width="100%" border="0" class="tablaPasos tablaVol">
                  <tr>
                    <td class="etiquetaPo">  <p>Recurso:</p></td>
                    <td>
                      &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="text" name="accionA" id="accionA" size="35" class="redonda5 camposEditar" value="<?php echo $accion ?>" readonly="readonly">
                    </td>
                  </tr>
                  </table> 
                  </div>
                        ...
                        <td>
                          ...
                          <option value="" >Seleccione un abogado...</option>
                          	<?php
								...
								{
									echo "<option value='".$a['usuario']."|".$a['nivel']."'>".$a['nombre']." | Nivel: ".$a['nivel']."</optiopn>";	
								}
							?>
                          </select>
                          </td>
