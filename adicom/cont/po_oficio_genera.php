...
if($TO == 0){
	...
	if($consecutivo <= 99 && $ultimo_id >= 10) $uid = "00".$consecutivo;
	if($consecutivo >= 100 && $ultimo_id < 1000) $uid = "0".$consecutivo;
	...
	for($i=0; $i<=count($partesAcciones); $i++)
	{
		if($partesAcciones[$i] != "")
		{
			if(stripos($oficioRef,"dgrr") !== false) {$tipo = "DGR"; }
	...
	echo $folio."|".$po."|".$cp."|".$ef."|".$mm."|".$ua."|".$di."|".$ca."|".$remNom."|".$remCar."|".$remDep."|".$fechaPO."|".$fojas."|".$fechaOficio."|".$horaOficio;
}// end TO
else
