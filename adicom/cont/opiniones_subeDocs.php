...
if (isset($_FILES)) 
{
  ...
  if (stripos($url, "adicom") !== false) $capeta = "/archivos/";
  ...
  if(move_uploaded_file($_FILES['adjunto']['tmp_name'],$_SERVER['DOCUMENT_ROOT'].$capeta.$nvoNomExt))
   {
	...
	/*	if($_POST['tipoDoc'] == "sicsa"){
			$query2 = "UPDATE opiniones_historial SET sicsa = '".$_POST['nomDoc']."', sicsaRecepcion = '".date("Y-m-d")."' , atendido = 1 WHERE oficio = '".$oficio."' AND num_accion = '".$_POST['accion']."'  ";
			$sql2 = $conexion->update($query2,false);
		}*/
		if($_POST['tipoDoc'] == "sicsa"){
			
			$query02 = "UPDATE oficios SET  atendido = '1' WHERE folio = '".$oficio."' AND num_accion like '%".$_POST['accion']."%'  ";
			$sql02 = $conexion->update($query02,false);
		}
