	  ...
	  select: function( event, ui ) {  
	   //reseteamos campos al seleccionar accion
		...
		if (ui.item.estado == 35) 
		{
			...
			$$("#remitente").val(ui.item.idact).attr('readonly',true);
			$$("#noacc").val(ui.item.acc).attr('readonly',true);
			$$("#oficioRef").val(ui.item.procedimiento).attr('readonly',true);	 
			$$("#dependencia").val(ui.item.dependencia).attr('readonly',true);
			$$("#asunto").val("Requerimientos al Recurrente.").attr('readonly', true);
 		}
		...
		if (ui.item.estado == 39) 
		{
			...
			$$("#oficioRef").val(ui.item.procedimiento).attr('readonly',true);	 
			$$("#dependencia").val(ui.item.dependencia).attr('readonly',true);
			$$("#asunto").val("Con fundamento en lo dispuesto por el artículo 70, I y II, párrafo segundo de la LFRCF, se admite a trámite el Recurso de Reconsideración interpuesto.").attr('readonly', true);
 		}
		...
		if (ui.item.estado == 39.1)
		{
			...
			$$("#oficioRef").val(ui.item.procedimiento).attr('readonly',true);	 
			$$("#dependencia").val(ui.item.dependencia).attr('readonly',true);
			$$("#asunto").val("Desechamiento del Recurso de Reconsideración.").attr('readonly', true);	
 		}
...
function cambiomsg()
{
	...
	else if($$("#tipoOficio").val() == "rr_not_resol_sat")
	{
		$$("#asunto").val("Se Notifica al SAT la Resolución del Recurso de Reconsideración.").attr('readonly', false);
	}
                        ...
                        <td>
                          <select name="tipoOficio" id="tipoOficio" class="redonda5" onclick="cambiomsg()" >
                          	<optgroup label="Proceso de la Acción">
                                <option value="NULL">Seleccione el tipo de oficio...</option>
                            <?php
